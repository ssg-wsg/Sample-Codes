...
class ViewSessionAttendance(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.label_runId = Label(self, text="Run Id*", width=20, font=("bold", 10), anchor = 'w')
        self.label_runId.place(x=100, y=105)

        label_runId_ttp = CreateToolTip(self.label_runId, tooltipDescription["CourseRunId"])

        self.entry_runId = Entry(self)
        self.entry_runId.place(x=250, y=105)
        ...
        self.label_sessionId = Label(self, text="Session Id*", width=20, font=("bold", 10), anchor = 'w')
        self.label_sessionId.place(x=100, y=155)

        label_sessionId_ttp = CreateToolTip(self.label_sessionId, tooltipDescription["SessionId"])

        self.entry_sessionId = Entry(self)
        self.entry_sessionId.place(x=250, y=155)

        # This method is used to update the display information dynamically in "Payload" Tab whenever user key in a value
        def typing(event):
            value = displayViewSession(self.entry_runId.get(),entry_TpUEN.get(),self.entry_crn.get(),self.entry_sessionId.get())
            curlText.delete("1.0", "end")
            curlText.insert(tk.END, value)

        self.entry_runId.bind('<KeyRelease>', typing)
        self.entry_crn.bind('<KeyRelease>', typing)
        self.entry_sessionId.bind('<KeyRelease>', typing)
        ...
        curlText.insert(tk.END, str(displayViewSession("",entry_TpUEN.get(),"","")))
        ...
        def displayResp(method):
            if method != 'encrypt':
                try:
                    display = ViewSessionAttendance.textResponse.get()
                except:
                    display = ''
                ...
                try:
                    display = doEncryption(str(ViewSessionAttendance.textResponse.get()).encode())
                except:
                    display = b''
                responseText.delete("1.0","end")
        ...
        def submitCallBack():
            responseText.delete("1.0", "end")
            resp = getSessionAttendance(self.entry_runId.get(),entry_TpUEN.get(),self.entry_crn.get(),self.entry_sessionId.get())
            ViewSessionAttendance.textResponse = StringVar(self, value=resp)
            responseText.insert(INSERT, ViewSessionAttendance.textResponse.get())
