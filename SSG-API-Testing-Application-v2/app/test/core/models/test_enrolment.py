...
from app.core.constants import (CollectionStatus, IdTypeSummary, SponsorshipType,
                                EnrolmentSortField, SortOrder, EnrolmentCourseStatus,
                                CancellableCollectionStatus)
from app.core.models.enrolment import (CreateEnrolmentInfo, UpdateEnrolmentInfo,
                                       CancelEnrolmentInfo, SearchEnrolmentInfo,
...
class TestEnrolmentInfo(unittest.TestCase):
    ...
    def test_CancelEnrolmentInfo_set_course_run_id(self):
        with self.assertRaises(ValueError):
            TestEnrolmentInfo.CANCEL_ENROLMENT_INFO.course_run_id = 123

        with self.assertRaises(ValueError):
            TestEnrolmentInfo.CANCEL_ENROLMENT_INFO.course_run_id = ["abc"]

        with self.assertRaises(ValueError):
            TestEnrolmentInfo.CANCEL_ENROLMENT_INFO.course_run_id = [1234]

        TestEnrolmentInfo.CANCEL_ENROLMENT_INFO.course_run_id = "10026"
        self.assertEqual(TestEnrolmentInfo.CANCEL_ENROLMENT_INFO._course_run_id, "10026")
        self.assertEqual(TestEnrolmentInfo.CANCEL_ENROLMENT_INFO._course_run_id,
                         TestEnrolmentInfo.CANCEL_ENROLMENT_INFO.course_run_id)
    ...
    def test_SearchEnrolmentInfo_payload(self):
        with self.assertRaises(AttributeError):
            TestEnrolmentInfo.SEARCH_ENROLMENT_INFO_ONE.payload()
    ...
    def test_UpdateEnrolmentFeeCollectionInfo_payload(self):
        with self.assertRaises(AttributeError):
            TestEnrolmentInfo.UPDATE_ENROLMENT_FEE_COLLECTION_ONE.payload()
