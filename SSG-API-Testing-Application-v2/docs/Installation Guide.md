...
> 
> If you do not have `git` installed, you can refer to [this guide](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git))
...
1. Open a new command prompt or terminal in a location that is convenient for you. You should take note that the location
...
If you do not have Docker installed in your environment, head over the [Docker Engine Installation Guide](https://docs.docker.com/engine/install/)
...
3. Run the following command to build the Docker container:
    ...
    docker build -t app .
    ```
   1. The `docker build` command will build the Docker container using the `Dockerfile` present in the current directory
      (folder)
